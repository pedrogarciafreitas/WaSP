...
minimal_config makeMinimalConfig(view *view0) 
{
	...
	//printf("%i,%i\t%i,%f\n", view0->r, view0->c, view0->sigma>0.001 ? 1 : 0, view0->sigma);

	min_conf.encoding_flags = view0->use_median ? min_conf.encoding_flags | (1 << 0) : min_conf.encoding_flags;
	min_conf.encoding_flags = view0->sigma>0.001 ? min_conf.encoding_flags | (1 << 1) : min_conf.encoding_flags;
...
void setup_form_minimal_config(minimal_config *mconf, view *view0) {
	...
	view0->sigma = ( mconf->encoding_flags & (1 << 1) )>0 ? (float)1.0 : (float)0.0;
